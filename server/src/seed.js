...
async function main() {
  ...
  // never wiped out.
  const alreadySeeded = (await get(`SELECT COUNT(*) AS n FROM customers`)).n > 0;
  if (alreadySeeded) {
  ...
  for (const r of userRows) {
    userIds.push((await run(`INSERT INTO users (name, email, role, initials, color) VALUES (@name, @email, @role, @initials, @color) RETURNING id`, r)).rows[0].id);
  }
  ...
  let bomCount = 0;
  for (let qi = 0; qi < quoteMeta.length; qi++) {
    const q = quoteMeta[qi];
  ...
  const woRows = [];
  wonQuotes.forEach((q, wn) => {
    const status = workOrderStatusCycle[wn % workOrderStatusCycle.length];
    woRows.push({
      quote_id: q.id,
      customer_id: customerIds[q.customerIdx],
      work_order_number: `WO-2026-${5000 + wn}`,
      status,
      owner_user_id: WALTER,
      start_date: '2026-08-' + String(18 + (wn % 10)).padStart(2, '0'),
      due_date: '2026-09-' + String(18 + (wn % 12)).padStart(2, '0'),
  ...
  extraWoSeed.forEach((e, wn) => {
    woRows.push({
      quote_id: null,
      customer_id: customerIds[e.c],
      work_order_number: `WO-2026-${6000 + wn}`,
      status: e.status,
      owner_user_id: WALTER,
      start_date: e.start,
      due_date: e.due,
  ...
  for (let sn = 0; sn < shipmentSeed.length; sn++) {
    const s = shipmentSeed[sn];
    const carrier = carriers[sn % carriers.length];
    await run(`
    ...
    `, {
      ...
      ship_date: s.ship,
      carrier,
      tracking_number: s.status !== 'Preparing' ? `${carrier.slice(0, 3).toUpperCase()}${100000000 + sn * 137}` : null,
      status: s.status,
    });
  }
  ...
  for (let inn = 0; inn < invoiceSeed.length; inn++) {
    const inv = invoiceSeed[inn];
    await run(`
    ...
    `, {
      ...
      invoice_number: `INV-2026-${7000 + inn}`,
