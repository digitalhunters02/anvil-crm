...
export const TONES = {
  neutral: { bg: '#e6e5e0', fg: '#4b4e51', border: '#d7d6cf' },
  brand: { bg: '#f5dcd8', fg: '#8f251d', border: '#e6bcb6' },
  amber: { bg: '#f3e7c9', fg: '#7a5c15', border: '#e3d19f' },
  rose: { bg: '#f5dbe4', fg: '#832149', border: '#e8b9cb' },
  blue: { bg: '#dbe8f2', fg: '#174e74', border: '#b8d3e6' },
  violet: { bg: '#e9def0', fg: '#5c3d7a', border: '#d5c2e2' },
  green: { bg: '#dcefdf', fg: '#255c2f', border: '#bbdcc1' },
  steel: { bg: '#e4e6e8', fg: '#3a4045', border: '#cdd1d5' },
};
...
export function CardHead({ title, sub, action }) {
  return (
    <div className="flex items-center justify-between px-5 pt-4 pb-3">
      <div>
        <h3 className="font-head uppercase tracking-wide text-[14px] font-semibold text-ink">{title}</h3>
...
export function Kpi({ label, value, sub, tone = 'neutral', icon }) {
  ...
  return (
    <Card className="p-5">
      ...
      <p className="font-head text-[27px] font-semibold text-ink mt-2 leading-none">{value}</p>
      {sub && <p className="text-xs text-muted mt-2">{sub}</p>}
    </Card>
  );
}

export function CellName({ primary, secondary, avatar, color, mono }) {
  return (
    <div className="flex items-center gap-2.5 min-w-0">
      {avatar && <Avatar name={primary} color={color} size={28} />}
      <div className="min-w-0">
        <p className={`text-sm font-medium text-ink truncate ${mono ? 'font-mono' : ''}`}>{primary}</p>
...
export function ConfirmDialog({ title = 'Delete record', message, error, busy, onCancel, onConfirm, confirmLabel = 'Delete' }) {
  return (
    <Modal
      title={title}
      onClose={onCancel}
      footer={
        <>
          ...
          <Button onClick={onConfirm} disabled={busy} className="!bg-rose hover:!bg-rose/90 text-white">
...
export function TextInput({ className = '', ...props }) {
  return <input className={`${fieldInputCls} ${className}`} {...props} />;
}

export function TextArea({ className = '', ...props }) {
  return <textarea className={`${fieldInputCls} ${className}`} rows={3} {...props} />;
}

export function SelectInput({ children, className = '', ...props }) {
  return (
    <select className={`${fieldInputCls} ${className}`} {...props}>
      {children}
    </select>
  );
}
